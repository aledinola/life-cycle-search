function [pi_semiz,N_semiz] = create_pi_semiz(pi_l,pi_g,n_s,n_l,n_g,N_j,N_i)
...
for s_c=1:n_s
    for z_c = 1:N_semiz
        [l_c,g_c] = ind2sub([n_l,n_g],z_c);
        for zp_c = 1:N_semiz
            [lp_c,gp_c] = ind2sub([n_l,n_g],zp_c);
            pi_semiz(z_c,zp_c,s_c,j_c,i_c) = pi_l(l_c,lp_c,s_c,j_c,i_c)*pi_g(g_c,gp_c,l_c);
        end
    end
end
end
