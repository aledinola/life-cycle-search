...
% Local compatibility shim for the toolkit simulation path.
% Some toolkit callers pass only six inputs; in that case default to
% requesting only the semi-exogenous transition probabilities.
...
if gridpiboth==3 || gridpiboth==1 || isfield(options,'SemiExoStateFn')
    % Regardless of whether we output semiz_gridvals_J, we sometimes have to create it as it is needed for evaluting SemiExoShockFn
